...
export function useAdvertisingData(preset: AdvertisingPreset, customRange?: AdvertisingRange) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (!active) return;
      const normalized = ((data as LegacyAdvertisingDailyRecord[] | null) ?? []).map(normalizeLegacyDailyMetric);
  ...
  const trend = useMemo(() => buildTrendPoints(metrics, range), [metrics, range]);
...
export function useAdvertisingAdDetail(adId: string, preset: AdvertisingPreset, customRange?: AdvertisingRange) {
  ...
  const filtered = useMemo(() => filterMetricsByRange(adMetrics, root.range), [adMetrics, root.range]);
  const summary = useMemo(() => summarizeMetrics(filtered), [filtered]);
  const skuMetrics = useMemo(() => buildSkuMetrics(ad.id, filtered), [ad.id, filtered]);
  const notes = useMemo(() => buildDailyNotes(ad.id, adMetrics), [ad.id, adMetrics]);
  const resolvedNotes = root.notesCrud.notesByAdId[ad.id]?.length ? root.notesCrud.notesByAdId[ad.id] : notes;
  const trend = useMemo(() => buildTrendPoints(adMetrics, root.range), [adMetrics, root.range]);
